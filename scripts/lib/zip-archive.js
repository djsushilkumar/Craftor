/**
 * Craftor POSIX PKZip Archive Writer
 * Zero-dependency deflate archive builder that always emits forward-slash ('/') entry
 * names, preventing the "Plugin file does not exist" error on Linux/macOS/cPanel hosts.
 */
...
/**
 * Recursively collects every file under `dir` as `{ fullPath, relPath }` entries,
 * where `relPath` is prefixed with `entryPrefix` and always uses forward slashes.
 */
function collectFiles(dir, entryPrefix, baseDir = dir) {
  let results = [];
  const entries = fs.readdirSync(dir, { withFileTypes: true });
  for (const entry of entries) {
    const fullPath = path.join(dir, entry.name);
    if (entry.isDirectory()) {
      results = results.concat(collectFiles(fullPath, entryPrefix, baseDir));
    } else {
      const relPath = path.relative(baseDir, fullPath).replace(/\\/g, '/');
      results.push({ fullPath, relPath: entryPrefix ? `${entryPrefix}/${relPath}` : relPath });
    }
  }
  return results;
}
...
/**
 * Builds a complete PKZip archive buffer (local headers, central directory, EOCD)
 * from `collectFiles` entries using raw deflate compression.
 */
function buildZipBuffer(files, date) {
  const localHeaders = [];
  ...
  return Buffer.concat([Buffer.concat(localHeaders), centralDirBuffer, eocd]);
}

module.exports = { crc32, getDosDateTime, collectFiles, buildZipBuffer };
