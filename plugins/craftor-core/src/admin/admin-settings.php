  ...
  "mcpServers": {
    ...
    public function render_wizard_page(): void {
            ...
            <div style="background: #fff; border: 1px solid #e2e8f0; border-radius: 16px; padding: 35px; box-shadow: 0 4px 20px rgba(0,0,0,0.06);">
                ...
                <div id="wizard-step-3" style="display: none;">
                    ...
                    <div id="wizard-loading-area" style="display: none; background: #0f172a; color: #fff; border-radius: 10px; padding: 20px; margin-bottom: 25px;">
                        ...
                        <div id="gen-step-1" style="font-size: 13px; color: #94a3b8; margin-bottom: 6px;">⏳ Communicating with Craftor Core REST Engine...</div>
        ...
        function selectArchetype(val, el) {
            ...
            if (titles[val]) {
                document.getElementById('wizard-site-title').value = titles[val];
            }
        }
        ...
        async function executeWizardBuild() {
            ...
            const apiToken = '<?php echo esc_js( $token ); ?>';

            document.getElementById('btn-start-build').disabled = true;
            document.getElementById('wizard-loading-area').style.display = 'block';

            try {
                const res = await fetch('<?php echo esc_url_raw( rest_url( "craftor/v1/wizard/generate" ) ); ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-WP-Nonce': restNonce,
                        'X-Craftor-Token': apiToken
                    },
                    body: JSON.stringify({
                        archetype: selectedArchetypeVal,
                ...
                if (data.success && data.page_id) {
                    ...
                    document.getElementById('wizard-success-title').innerText = '🎉 "' + data.title + '" is Live!';
                    document.getElementById('wizard-success-desc').innerText = 'Generated with ' + data.containers_count + ' native Elementor containers for archetype: ' + data.archetype.toUpperCase();
                    document.getElementById('btn-view-site').href = data.page_url;
                    document.getElementById('btn-edit-elementor').href = data.editor_url;
                } else {
                    alert('Error creating website: ' + (data.message || 'Unknown error'));
